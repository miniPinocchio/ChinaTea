...
public class AbStringUtils {
    ...
    private final static Pattern PHONE =
            Pattern.compile("^1\\d{10}$");

    private final static Pattern IMG_URL =
    ...
    public final static Pattern LEGAL_NAME =
            Pattern.compile("^[a-zA-Z_0-9\\.`~!@#$%^&*()+-=/\\u4e00-\\u9fa5]{4,20}$");

    public final static Pattern LEGAL_PASSWORD =
            Pattern.compile("^[a-zA-Z_0-9\\.`~!@#$%^&*()+-=/]{6,20}$");

    public final static Pattern IDCARD =   //身份证号验证
     ...
     * $ 匹配行结尾位置
     */
    public final static Pattern LEGAL_NUMBER_AND_LETTER_PASSWORD =
     ...
     * @return
     */
    public static boolean isLegalNumberAndLetterPassword(String password) {
        return !isEmpty(password) && LEGAL_NUMBER_AND_LETTER_PASSWORD.matcher(password).matches();
    }
    ...
    public static boolean isIdCard(String idcard) {
        return !isEmpty(idcard) && IDCARD.matcher(idcard).matches();
    }
